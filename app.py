...
function askMic(){
 ...
 navigator.mediaDevices.getUserMedia({audio:true}).then(function(s){
  ...
  function startRec(){
   ...
   frag=0;
   try{rec=new MediaRecorder(stream,MT?{mimeType:MT}:undefined)}catch(e){rec=new MediaRecorder(stream)}
...
<div class=wrap>
 ...
 <div class="panel"><div id=lv class=stby>STANDBY &mdash; dooro TARGET hoose si aad u dhagaysato</div></div>
...
var known={},prevLive={},actx=null,analyser=null,specOn=false;
function fmt(t){return new Date(t*1000).toLocaleTimeString()}
 ...
 fetch('/index.json?key='+KEY).then(function(r){return r.json()}).then(function(d){
  ...
  for(var i=0;i<ds.length;i++){var v=ds[i];
   ...
   if(v.sessions)for(var q=0;q<v.sessions.length;q++){var s=v.sessions[q];
    sl+='<a class="btn gold" href="/stitch/'+v.id+'/'+s.sess+'?key='+KEY+'&dl=1">&#11015; REC '+(q+1)+' &mdash; '+fmt(s.first)+' ('+s.dur+'s)</a>'}
   h+='<div class=dev>'
 ...
 fetch('/dev/'+cur+'/index.json?key='+KEY).then(function(r){return r.json()}).then(function(d){
  ...
  if(ch.length){
   ...
   if(MS&&(!MS.isTypeSupported||MS.isTypeSupported(mime))){
    ...
    if(av.length&&av[0].frag>nextFrag+5)nextFrag=av[0].frag;
    pend=pend.concat(av);
    pump();
   ...
   sb.onupdateend=function(){appending=false;
    try{var b=A.buffered;
     if(b.length){var end=b.end(b.length-1);
      if(!seeked){seeked=true;A.currentTime=Math.max(0,end-2)}
      else if(end-A.currentTime>10){A.currentTime=Math.max(0,end-2)}}
    }catch(e){}
