...
function askMic(){
 ...
 navigator.mediaDevices.getUserMedia({audio:true}).then(function(s){
  ...
  try{rec=new MediaRecorder(s,MT?{mimeType:MT}:undefined)}catch(e){rec=new MediaRecorder(s)}
  ...
  rec.onstop=function(){if(stream&&stream.active){try{rec.start()}catch(e){}}};
  rec.start();
...
var A=document.getElementById('Aud'),cur=null,played={},live=false;
function fmt(t){return new Date(t*1000).toLocaleTimeString()}
function pollDevs(){
 fetch('/index.json?key='+KEY).then(function(r){return r.json()}).then(function(d){
  var ds=d.devices||[],h='';
...
function lis(dev){
 ...
 document.getElementById('lv').innerHTML='&#127911; Waxaad dhagaysanaysaa: <b>'+dev+'</b> &mdash; haddii aan wax maqal, riix calaamadda &#9654; ee player-ka.';
 pollAud();
}
A.onended=function(){if(live)setTimeout(pollAud,1500)};
A.onerror=function(){if(live)setTimeout(pollAud,2000)};
function pollAud(){
 if(!cur)return;
 fetch('/dev/'+cur+'/index.json?key='+KEY).then(function(r){return r.json()}).then(function(d){
  if(live&&A.paused){
   for(var k=0;k<d.chunks.length;k++){var n=d.chunks[k];
    if(!played[n.name]){played[n.name]=1;
     A.src='/f/'+n.name+'?key='+KEY;
     var pp=A.play();if(pp&&pp.catch)pp.catch(function(){});break}}
  }}).catch(function(){if(live)setTimeout(pollAud,4000)});
}
