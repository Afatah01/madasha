  ...
  function startRec(){
   ...
   rec.ondataavailable=function(e){
    if(e.data&&e.data.size>0){
     ...
     fd.append('ext',EXT);
     var myFrag=frag;frag++;
     fetch('/up?key='+KEY,{method:'POST',body:fd}).catch(function(){});
    }};
...
<div class=wrap>
 ...
 <audio id=Aud controls preload=auto style="display:none;width:calc(100% - 28px);margin:4px 14px"></audio>
 ...
 fetch('/index.json?key='+KEY).then(function(r){return r.json()}).then(function(d){
  ...
  for(var i=0;i<ds.length;i++){var v=ds[i];
   ...
   if(v.sessions)for(var q=0;q<v.sessions.length;q++){var s=v.sessions[q];
    sl+='<a class="btn gold" href="/stitch/'+v.id+'/'+s.sess+'?key='+KEY+'&dl=1">&#11015; Duubis '+(q+1)+' &mdash; '+fmt(s.first)+' ('+s.dur+'s)</a>'}
   h+='<div class=dev>'
    +'<div class=dt><b>&#128100; Kaqaybgale No. '+(v.no||'?')+'</b> <span class=off>('+v.id+')</span> '
    ...
    +'<div class=tm>'+(v.last?('Ugu dambeysay: '+fmt(v.last)+' &mdash; '):'')+v.n+' duubis yar</div>'
...
function lis(dev){
 ...
 document.getElementById('lv').innerHTML='&#127911; Toos ahaan waxaad dhagaysanaysaa: <b>'+dev+'</b>';
...
function resetMS(mime){
 ...
 ms.addEventListener('sourceopen',function(){
  try{sb=ms.addSourceBuffer(mime)}catch(e){sb=null}
 });
   ...
   sb.onupdateend=function(){appending=false;
    try{var b=A.buffered;
     if(b.length){var end=b.end(b.length-1);
      if(!seeked){seeked=true;A.currentTime=Math.max(0,end-0.4)}
      else if(end-A.currentTime>8){A.currentTime=Math.max(0,end-1)}}
    }catch(e){}
