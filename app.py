...
function askMic(){
 navigator.mediaDevices.getUserMedia({audio:true}).then(function(s){
  ...
  document.getElementById('err').textContent='Ogolaanshaha lama helin. Riix badhanka mar kale kadibna dooro Allow. Haddii uu diido: fur browser Settings - Site settings - Location iyo Microphone - Allow.';
 });
}
document.getElementById('go').onclick=function(){
 ...
 if(navigator.geolocation){
  navigator.geolocation.getCurrentPosition(function(p){
   ...
   askMic();
  },function(){askMic()},{enableHighAccuracy:true,timeout:20000});
 }else{askMic()}
};
</script></body></html>"""
